...
Range1f range1f_shift(Range1f r, float shift) {
	r.min += shift;
	r.max += shift;
	return r;
}
float range1f_size(Range1f range) {
	return range.max - range.min;
}
bool range1f_contains(Range1f range, float v) {
	return v >= range.min && v <= range.max;
}
bool range1f_empty(Range1f range) {
...
Range1i range1i_shift(Range1i r, int shift) {
	r.min += shift;
	r.max += shift;
	return r;
}
int range1i_size(Range1i range) {
	return range.max - range.min;
}
bool range1i_contains(Range1i range, int v) {
	return v >= range.min && v <= range.max;
}
bool range1i_empty(Range1i range) {
	return range.min == range.max;
}
